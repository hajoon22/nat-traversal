...
static ssize_t build_icmp_error(uint8_t **buf, uint8_t *inner_packet, size_t inner_len, uint8_t type, uint8_t code) {
...
int send_icmp_error(struct nt_session *nts, struct nt_send_packet *pkt, uint8_t type, uint8_t code) {
    ...
    uint8_t *buf = NULL;
    ssize_t len = build_icmp_error(&buf, inner_packet, inner_len, type, code);
    free(inner_packet);
    if (len < 0) {
        return (int)len;
    }
